...
abstract class Generator extends Model
{
    ...
    /**
     * @var int witch template type to use
     * The value of this property is internally managed by this class.
     ...
     * Simplifies creating of available file types drop-down list.
     */
     ...
     * Return a file extension string depending on configured file type.
     */
    protected function getFileExtension()
    {
        $types = self::$available_file_types;
        if (isset($types[$this->templateType])) {
            return $types[$this->templateType];
        } else {
            return $types[self::TEMPLATE_TYPE_PHP];
        }
    }
     ...
     * Please refer to [[\aayaresko\gii\generators\controller\Generator::generate()]] as an example
    ...
    public function generateString($string = '', $placeholders = [])
    {
        ...
        if ($this->enableI18N) {
            ...
            $str = "{$function}('" . $this->messageCategory . "', '" . $string . "'" . $ph . ")";
        } else {
